...
def get_employees_page(page: int, per_page: int):
    with SessionLocal() as session:
        stmt = (
            select (Employee).offset((page-1)*per_page).limit(per_page)
        )
        return session.execute(stmt).scalars().all()
...
def get_employee_with_optional_departments():
    with SessionLocal() as session:
        stmt = (
...
def get_project_employees_departments():
    with SessionLocal() as session:
        stmt = ( 
            select( Project.name.label("project_name"),
...
def count_employees_by_departament():
    with SessionLocal() as session:
        stmt = (
            select(Department.name,func.count(Employee.id)).join(Empployee,Employee.department_id == Department.id).group_by(Department.name)
        )
        return session.execute(stmt).all()
...
def get_average_salary_by_department():
    with SessionLocal() as session:
        stmt = (
            ...
            ).join(Employee,Employee.department_id == Department.id).group_by(Department.name)
        )
        return session.execute(smt).all()
